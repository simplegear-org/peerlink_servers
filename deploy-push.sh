...
install_base_packages() {
  if [[ "${PEERLINK_PUSH_SKIP_SYSTEM_SETUP:-false}" == "true" ]]; then
    return 0
  fi
...
install_docker() {
  ...
  curl -fsSL \
    ...
    -o /tmp/peerlink-docker-install/get-docker.sh
...
check_dns_points_here() {
  ...
  resolved_ips="$(
    getent ahosts "$PUSH_PUBLIC_HOST" 2>/dev/null \
      | awk '{print $1}' \
      | sort -u \
      || true
  )"
...
check_public_ports() {
  if compose ps -q push-proxy >/dev/null 2>&1 \
    && [[ -n "$(compose ps -q push-proxy 2>/dev/null || true)" ]]; then
    return
  fi
  ...
  listeners="$(
    ss -ltn '( sport = :80 or sport = :443 )' 2>/dev/null \
      | tail -n +2 \
      || true
  )"

  if [[ -n "$listeners" ]]; then
    echo "Ports 80/443 appear to be in use."
    echo "Stop the conflicting service before deploy."
    echo "$listeners"
    exit 1
  fi
}
...
validate_fcm_credentials_json() {
  if ! compose run \
    ...
    '
  then
    echo "FCM_CREDENTIALS_JSON must be valid JSON."
    echo "Wrap it in single quotes in .env.push.local."
    exit 1
  fi
}
...
wait_for_push_ready() {
  local attempts="${PUSH_READY_ATTEMPTS:-30}"
  local delay="${PUSH_READY_DELAY_SECONDS:-2}"
  local attempt

  echo "Waiting for push service readiness..."
  ...
  echo "Push service did not become ready."
...
wait_for_proxy_ready() {
  local attempts="${PUSH_PROXY_READY_ATTEMPTS:-30}"
  local delay="${PUSH_PROXY_READY_DELAY_SECONDS:-2}"
  local attempt

  echo "Waiting for push proxy readiness..."
  ...
  for ((attempt = 1; attempt <= attempts; attempt++)); do
    if compose exec -T push-proxy \
      wget \
        -q \
        -O /dev/null \
        http://peerlink_push_backend/health \
      >/dev/null 2>&1
    then
      echo "Push proxy can reach push backend."
      return 0
    fi
  ...
  echo "Push proxy cannot reach push backend."
...
restart_proxy_after_upstreams_ready() {
  wait_for_push_ready

  #
  # Dynamic Docker DNS via resolver + upstream resolve should already
  # protect nginx against container IP changes.
  #
  # Restart here additionally guarantees a clean config activation after
  # deploy and avoids retaining any state from an older nginx process.
  #
...
main() {
  ...
  if uses_cloudflare_origin_tls; then
    ...
    #
    # Never replace/restart production nginx with syntactically invalid config.
    #
    validate_nginx_config
    ...
    #
    # Certificate now exists, so validate complete TLS config before activation.
    #
    validate_nginx_config
