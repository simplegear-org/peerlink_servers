...
- if group membership is unknown yet, relay bootstraps initial membership as:
  `from + recipients[]`
- bootstrapped membership is provisional; a later signed
  `/relay/group/members/update` from the real owner can replace that owner

Push behavior:
- relay does not send push itself
- clients send `group_update` through the push service only after a successful
  group relay store
...
has been accepted, further updates with a different `ownerPeerId` are rejected
with `409 owner mismatch`.
...
- blob upload is storage-only and does not enforce group membership
- group delivery authorization is enforced later by `/relay/group/store`
...
- chunk upload is storage-only and does not enforce group membership
- group delivery authorization is enforced later by `/relay/group/store`
...
- chunk completion is storage-only and does not enforce group membership
- group delivery authorization is enforced later by `/relay/group/store`
